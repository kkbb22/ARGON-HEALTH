...
- **Security / Authorization / RLS / Tenant Isolation** — the mandatory
  ...
  logic were bypassed. **Named explicitly (closes GAP-013 in
  `docs/audit/FINAL-GAP-ANALYSIS.md`), not left implicit within the
  general ALLOW/DENY discipline:** (a) **privilege escalation** — DENY
  any actor from granting themselves a role/entitlement above their
  current one, mirroring the cross-tenant-read DENY case in rigor; (b)
  **break-glass** — ALLOW invocation only with MFA + reason + time-box
  present (`07`), DENY invocation without any one of the three, and a
  separate test asserting a break-glass session auto-expires and cannot
  be silently renewed.
...
## Related ADRs
None — the GAP-013 privilege-escalation/break-glass named test
categories (2026-09-02, under ADR-020) elaborate the existing Security
layer discipline; no new technology or architecture decision required.
