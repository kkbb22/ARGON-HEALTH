...
Map**
Status: Proposed. Context: full 15-field depth for all 40 domains and 30
...
Status: Proposed — **RESOLVED by ADR-019** (`docs/adr/ADR-019-RECONCILIATION-WITH-ADR-000.md`,
2026-08-27). Context: this master foundation's own prompt specifies Java
25 LTS / Spring Boot 4.1 / Spring Modulith / PostgreSQL 18 (`14`); a prior
architecture consultation for this same project specified a different
Java version and left the Kotlin/Spring-Boot-vs-NestJS choice open — and,
separately, an existing governing document (`ADR-000`, the
`argon-platform-target-architecture` skill, dated 2026-08-22) already
established that ARGON's live Firebase system evolves only via staged,
trigger-based migration, never a big-bang rewrite. Decision: this
document set's Java 25/Spring Boot baseline is retained as the *updated
technical content* of ADR-000's North Star, but is now explicitly
subordinate to ADR-000's Staged Trigger Table and AI Agent Rules — see
ADR-019 for the full reconciliation, including why a 2026-08-23 blocked/
defective rebuild attempt on an earlier version of this same stack makes
ADR-000's staging discipline non-optional. Consequences: no phase of
`20-MASTER-IMPLEMENTATION-ROADMAP.md` may be treated as authorized to
start merely because it exists in this document set — each phase now
carries an explicit ADR-000 trigger cross-reference.
...
**ADR-019 — Reconciliation with ADR-000 (ARGON Platform Target
Architecture & Migration Strategy)**
- Status: **PROPOSED**
- Date: 2026-08-27
- Context: An existing, previously-established governing document
  (`ADR-000`, `argon-platform-target-architecture` skill, dated
  2026-08-22, "Accepted as North Star") already reconciles ARGON's
  long-term vision against `argon-governance`'s "reality over ambition"
  principle via a Staged Trigger Table, a "build now" list, and explicit
  AI Agent Rules against proposing a Firebase migration without a fired
  trigger. This entire v2/v3 `docs/` tree (ADR-014) was authored without
  referencing it. Separately, a real rebuild attempt on 2026-08-23 — three
  days prior, on an earlier version of this same stack — was found
  blocked with a non-functional build environment and a forensic audit
  finding real code defects.
- Decision: ADR-000 remains the governing document unchanged; this
  document set's technology baseline supersedes ADR-000's inherited
  NestJS reference as updated technical content only; every phase in
  `20-MASTER-IMPLEMENTATION-ROADMAP.md` is re-gated by ADR-000's trigger
  table; a new permanent build-verification-before-implementation-claim
  gate (Rule L15) is added, sourced directly from the 2026-08-23 failure;
  the live Firebase system remains the sole production system.
- Full detail: `docs/adr/ADR-019-RECONCILIATION-WITH-ADR-000.md`.
- Consequences: ADR-014 is now marked RESOLVED. No implementation is
  authorized by this reconciliation — the next real, low-risk step
  toward the vision is ADR-000's own "build now" list, inside the
  existing Firebase schema, not this stack.
- Revisit Trigger: Any ADR-000 trigger-table row firing for the first
  time.

**ADR-020 — Deliberate Freeze Exception: Gap Analysis Pass (2026-09-02)**
- Status: **PROPOSED**
- Date: 2026-09-02
- Context: `docs/governance/ARCHITECTURE-FREEZE.md` defines exactly five
  valid triggers for reopening the frozen baseline (security
  vulnerability, regulatory/legal change, technology EOL/critical
  change, measured production evidence, material new requirement). A
  request for a full gap-analysis-and-completion pass across the entire
  repository was made without citing any of the five. This was raised
  explicitly to the requester before proceeding.
- Decision: The requester, informed of the freeze and the five-trigger
  rule, made a conscious, explicit choice to proceed anyway: "نفذ الـ
  Gap Analysis كاملة واعياً إنه فتح للتجميد" ("execute the gap analysis
  fully, aware this reopens the freeze"). This ADR records that
  choice as the deliberate exception it is, rather than either (a)
  silently treating a sixth "final" pass as routine, or (b) refusing a
  repository owner's explicit, informed decision about their own
  project.
- Rationale: A freeze process that cannot be consciously overridden by
  its own owner is not governance, it's a lock without a key. The
  discipline that matters is that the override is *logged and
  reasoned*, not that it never happens.
- Consequences: `docs/audit/FINAL-GAP-ANALYSIS.md`,
  `docs/audit/FINAL-GAP-REGISTER.md`, `docs/audit/DO-NOT-BUILD-YET.md`,
  and `docs/audit/FINAL-IMPLEMENTATION-READINESS.md` are added under
  this exception. **This ADR does not re-open the freeze generally** —
  `docs/governance/ARCHITECTURE-FREEZE.md` remains in force for every
  future request; each future exception requires its own explicit,
  logged instance of this same conscious choice, not a standing license
  to keep analyzing indefinitely.
- Security/Operational/Compliance Impact: None — this pass adds
  analysis documents only, per the originating task's own Section 30
  Hard Stop Rule (no implementation, no Firebase migration, no
  infrastructure deployment, no production change).
- Revisit Trigger: N/A — this ADR documents a single, dated exception
  event.
