...
- **Template Cloning & Versioning** — closes GAP-012 in
  `docs/audit/FINAL-GAP-ANALYSIS.md`. Templates (Clinic / Medical Center
  / Complex / Hospital / etc., per the TEMPLATE step) are versioned;
  every Organization's provisioning record pins the exact template
  version resolved at its own CREATE time. A later change to a
  template's definition never silently re-applies to already-provisioned
  Organizations — propagating a template update to existing
  Organizations is itself a separate, explicit, audited operation, not
  an automatic side effect of editing the template. Cloning (seeding a
  new Organization's TEMPLATE/STRUCTURE/MODULES/POLICIES from an
  existing Organization's already-resolved configuration, instead of a
  blank template) is a documented saga variant: same CREATE→...→ACTIVATE
  sequence, with the STRUCTURE/MODULES/POLICIES steps' input sourced
  from the reference Organization's current state rather than the
  template defaults.
...
## Related ADRs
None — the GAP-012 Template Cloning & Versioning closure (2026-09-02,
under ADR-020) elaborates the existing saga; no new technology or
