...
- **Patient App** — read-mostly (Patient 360 subset per `10`), plus
  ...
  offline writes. **Account recovery and device/session management**
  (closes GAP-008 in `docs/audit/FINAL-GAP-ANALYSIS.md`): a lost-password
  or lost-MFA-device recovery flow requires step-up verification (never
  a bare email link alone, given the sensitivity of what it protects) —
  the specific verification method (e.g., a secondary channel confirmed
  at registration) is deferred to Phase 1 detailed design, not invented
  here. Patients can view and revoke their own active sessions/devices
  from within the app (the same server-side session-revocation mechanism
  `07` already defines for every session, exposed as a self-service
  view) — losing a phone means the patient can immediately revoke that
  device's session without contacting support.
...
## Related ADRs
None — the GAP-008 Patient App account-recovery/session-management
closure (2026-09-02, under ADR-020) elaborates the existing app
inventory; no new technology or architecture decision required.
