...
## Related ADRs
ADR-017 (IaC: OpenTofu replaces Terraform —
`docs/adr/ADR-019-RECONCILIATION-WITH-ADR-000.md` is unrelated to this
one; ADR-017's full form lives in `19-MASTER-ARCHITECTURAL-DECISIONS.md`
only), ADR-003 (Cloud Run over Kubernetes, re-confirmed not changed).
