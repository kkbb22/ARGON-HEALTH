...
*(closes the architectural half of GAP-009 — HIGH severity in
`docs/audit/FINAL-GAP-ANALYSIS.md`; the jurisdiction-specific parameters
remain explicitly open — see below, not invented here)*
- **Clinical records are not subject to a general "right to erasure."**
  Unlike ordinary personal data under GDPR-style frameworks, clinical
  records in most jurisdictions are subject to mandatory
  medical-record-retention law that overrides a patient's deletion
  request for a defined statutory period. The architecture's mechanism
  for a patient exercising a data-subject right against clinical data is
  therefore **restriction of processing / anonymization of
  non-mandatory fields**, not hard deletion — the same distinction `03`'s
  `Clinical` domain already draws for amendments ("no silent mutation of
  finalized clinical data").
- **PHI, Financial, and Operational tiers** (above) are retained per
  jurisdiction-specific statutory minimums. **The exact retention
  period per tier per jurisdiction is UNKNOWN — REQUIRES LEGAL
  VERIFICATION**, tracked as new rows in
  `docs/compliance/COMPLIANCE-TRACEABILITY-MATRIX.md` (one row per
  jurisdiction × record-category) — this document defines the mechanism,
  not the numbers, consistent with `08`'s existing status discipline.
- **Legal hold** — a flag settable on any record or record-set (Control
  Plane operator action, `09`, audited like any high-risk operation)
  that suspends any scheduled retention/archival/anonymization action
  for that record regardless of its normal retention timer. A record
  under legal hold is otherwise unaffected — reads/writes proceed
  normally under the usual authorization rules; only the
  retention/deletion *lifecycle* is frozen.
- **Reference and Telemetry tiers** are not subject to patient
  data-subject rights (they contain no PHI by construction) — standard
  operational retention (versioned/rolling) applies, no legal-hold
  mechanism needed for these two tiers specifically.
...
### Tenant Isolation Beyond Synchronous Requests
*(closes GAP-005 — HIGH severity in `docs/audit/FINAL-GAP-ANALYSIS.md`.
RLS is a per-*query* mechanism; these three categories don't get it for
free and need their own explicit statement.)*
- **Background jobs** — one job invocation is always scoped to exactly
  one tenant, running under that tenant's RLS-scoped role. There is no
  "cross-tenant batch job" that queries multiple tenants' data in a
  single execution context — a platform-wide scheduled task (e.g., a
  nightly report) fans out into N single-tenant job invocations, never
  one job that iterates tenants internally with an elevated role.
- **Exports** — an export (bulk or single-record) goes through the same
  Authorization Service check as any other read (`07`'s Authorization
  Model) — an export code path is not a separate, RLS-bypassing
  privileged path. Bulk exports additionally follow `07`'s Bulk Access
  Control tier.
- **Cache keys** — every Redis key is namespaced `{tenantId}:{rest-of-key}`
  by construction, enforced by the platform's shared caching helper
  (`Platform` domain, `03`) — no domain constructs a cache key manually
  without going through this helper, preventing an accidental
  un-namespaced key from leaking one tenant's cached value into another
  tenant's read path.
...
## Related ADRs
None — GAP-005/GAP-009 closures (2026-09-02, under ADR-020) elaborate
existing architecture; no new technology decision required.

## Unknowns
UNKNOWN — REQUIRES LEGAL VERIFICATION: exact retention period per data
tier per jurisdiction (see Retention, Legal Hold, and Deletion, above).
