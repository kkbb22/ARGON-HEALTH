...
- [x] Domain map reconciled — all 40 domains from the governance task's
...
- [x] Every major decision has an explicit status — all 19 ADRs in
      `docs/master/19` (including ADR-019's reconciliation with ADR-000)
      plus ADR-016/017/018/019's full form in `docs/adr/` are marked
      PROPOSED (none silently marked APPROVED — verified via
