...
|---|---|---|---|---|
| L1 | No module/domain without an owner | Prevents orphaned responsibility | Lint: every domain entry in `03` must resolve to a named role in its Permissions field | PASS — all 40 domains have a Permissions field naming a role |
...
| L15 | No phase may be marked IMPLEMENTED without (a) a pre-verified functional build environment and (b) an adversarial/forensic code audit — never a self-reported "tests passed" alone | Sourced directly from the 2026-08-23 blocked/defective rebuild attempt (see `docs/adr/ADR-019-RECONCILIATION-WITH-ADR-000.md`) — a real, dated failure of exactly this shortcut on this project | Lint (future): a phase's exit criteria in `docs/master/20` must show both a build-environment verification artifact and an independent audit artifact before status can move past IMPLEMENTED | N/A — no phase has been implemented yet; rule recorded for future enforcement |
