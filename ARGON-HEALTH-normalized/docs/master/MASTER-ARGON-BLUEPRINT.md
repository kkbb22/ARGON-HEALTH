# ARGON HEALTH PLATFORM — MASTER BLUEPRINT
...
## 7. Domain Map
39 domains, 15-field profiles, tiered by blast radius —
...
## 19. Interoperability
FHIR R4/R4B (production baseline, ADR-018), R5 conditional per specific
partner requirement, R6 tracked only; HL7 v2, DICOM/DICOMweb, IHE
profiles, all behind one Interoperability Layer —
...
Storage, Secret Manager/KMS, Cloud Armor/Cloudflare, OpenTofu (ADR-017,
replaces Terraform), Google Cloud Pub/Sub primary event backbone
(ADR-016), GitHub Actions — `13-MASTER-INFRASTRUCTURE-ARCHITECTURE.md`.
...
- Technology-stack version drift against any earlier ARGON decision is
  unreconciled (ADR-014, `19`).
...
- Scope is large — 39 domains, 30 workflows, 8+ target jurisdictions;
  the roadmap (`20`) exists specifically to prevent building all of it
  speculatively ahead of real demand.

## 34. Open Decisions
- Whether/how this target reconciles with any prior ARGON architecture
  decision (ADR-014, `19`) — unresolved by design, flagged for explicit
  follow-up.
